...
namespace VKPHPUtils\Mapping\Generator\Transformation\ToCollection\ToIteratorAggregate;
...
abstract class AbstractToIteratorAggregateTransformation implements ITransformation
{
    ...
    public function transform(
        ...
        return $this->anonymousIteratorAggregate($expr, $reflectionMethod);
    }

    protected function anonymousIteratorAggregate(Expr $expr, ReflectionMethod $reflectionMethod): Expr
    {
        $builderFactory = new BuilderFactory();
        return new New_(
            new Class_(
                null, [
                'implements' => [$builderFactory->name(IteratorAggregate::class)],
                'stmts' => [

                    $builderFactory->method('__construct')
                        ->makePublic()
                        ->addParams(
                            [
                            $builderFactory->param('mapper')->makePrivate()->setType(Constants::MAPPER_CLASS_NAME),
                            $builderFactory->param('data')->makePrivate()->setType($builderFactory->name('iterable'))
                            ]
                        )
                        ->addStmts([])
                        ->getNode(),

                    $builderFactory->method('getIterator')
                        ->makePublic()
                            ...
                            $builderFactory->return(
                                new New_(
                                    $this->anonymousIterator($reflectionMethod->class, $reflectionMethod->name), [
                                        $builderFactory->argument($builderFactory->propertyFetch($builderFactory->this(), 'mapper')),
                                        $builderFactory->argument($builderFactory->propertyFetch($builderFactory->this(), 'data')),
                                    ]
                                )
                            )
                        )
